...
class Solver(object):
    ...
    def build_model(self):
        ...
        self.g_optimizer = optim.Adam(
            self.generator.parameters(), self.lr, betas=(self.beta1, self.beta2)
        )
        self.d_optimizer = optim.Adam(
            self.discriminator.parameters(), self.lr, betas=(self.beta1, self.beta2)
        )
            ...
            for i, (data, _) in enumerate(self.data_loader):
                ...
                if self.loss == "original":
                    d_loss_real = self.criterion(
                        outputs_real.squeeze(), real_labels.squeeze()
                    )
                    d_loss_fake = self.criterion(
                        outputs_fake.squeeze(), fake_labels.squeeze()
                    )
                    ...
                    d_loss = (
                        -outputs_real.mean() + outputs_fake.mean() + gradient_penalty
                    )
                ...
                if self.loss == "original":
                    g_loss = self.criterion(
                        outputs_fake.squeeze(), real_labels.squeeze()
                    )
                elif self.loss == "wgan-gp":
                ...
                if (i + 1) % self.sample_step == 0:
                    save_image(
                        denorm(fake_data).cpu(),
                        self.sample_path + "/epoch_{}_{}.png".format(i + 1, epoch + 1),
                        nrow=8,
                    )
                ...
                if (i + 1) % self.validation_step == 0:
                    ...
                    for j in range(self.num_imgs_val // batch_size):
                        fake_data_all[j * batch_size : (j + 1) * batch_size] = to_numpy(
                            fake_data
                        )
                    ...
                    if fid_value < self.fid_score:
                        ...
                        d_path = os.path.join(
                            self.model_path, "discriminator-best-fid.pkl"
                        )
                        torch.save(self.generator.state_dict(), g_path)
                        torch.save(self.discriminator.state_dict(), d_path)
